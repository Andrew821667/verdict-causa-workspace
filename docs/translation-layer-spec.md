...
`TranslationTemplateSet` содержит ровно один шаблон каждого уровня, локаль `ru-RU`, версию и SHA-256 hash канонического JSON. Версия `translation-template-ru-v7` добавляет профессиональное объяснение и forensic-трассировку способов обеспечения исполнения по статьям 329–381.2 ГК РФ поверх модели недействительности `ru-v6`. Активная политика хранит одновременно:

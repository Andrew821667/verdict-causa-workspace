...
`TranslationTemplateSet` содержит ровно один шаблон каждого уровня, локаль `ru-RU`, версию и SHA-256 hash канонического JSON. Версия `translation-template-ru-v9` добавляет профессиональное объяснение и forensic-трассировку исполнения и средств защиты по статьям 309–328 и 393–406.1 ГК РФ поверх модели динамики обязательств `ru-v8`. Активная политика хранит одновременно:

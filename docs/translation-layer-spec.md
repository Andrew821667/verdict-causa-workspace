...
`TranslationTemplateSet` содержит ровно один шаблон каждого уровня, локаль `ru-RU`, версию и SHA-256 hash канонического JSON. Версия `translation-template-ru-v10` добавляет профессиональное объяснение и forensic-трассировку специальных правил поставки по статьям 506–524 ГК РФ поверх модели исполнения и средств защиты `ru-v9`. Активная политика хранит одновременно:

...
`TranslationTemplateSet` содержит ровно один шаблон каждого уровня, локаль `ru-RU`, версию и SHA-256 hash канонического JSON. Версия `translation-template-ru-v3` добавляет профессиональное резюме ответственности и forensic-трассировку формальных предпосылок статей 333 и 401 ГК РФ поверх контрфактических разделов `ru-v2`. Активная политика хранит одновременно:

...
`TranslationTemplateSet` содержит ровно один шаблон каждого уровня, локаль `ru-RU`, версию и SHA-256 hash канонического JSON. Версия `translation-template-ru-v8` добавляет профессиональное объяснение и forensic-трассировку перемены лиц и прекращения обязательств по статьям 382–419 ГК РФ поверх модели обеспечения `ru-v7`. Активная политика хранит одновременно:

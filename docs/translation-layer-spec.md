...
`TranslationTemplateSet` содержит ровно один шаблон каждого уровня, локаль `ru-RU`, версию и SHA-256 hash канонического JSON. Версия `translation-template-ru-v6` добавляет профессиональное объяснение и forensic-трассировку недействительности сделок по статьям 166–181 ГК РФ поверх модели изменения и расторжения `ru-v5`. Активная политика хранит одновременно:

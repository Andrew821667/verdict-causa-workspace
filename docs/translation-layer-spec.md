...
`TranslationTemplateSet` содержит ровно один шаблон каждого уровня, локаль `ru-RU`, версию и SHA-256 hash канонического JSON. Версия `translation-template-ru-v5` добавляет профессиональное объяснение и forensic-трассировку изменения и расторжения договора по статьям 450–453 ГК РФ поверх модели заключения `ru-v4`. Активная политика хранит одновременно:

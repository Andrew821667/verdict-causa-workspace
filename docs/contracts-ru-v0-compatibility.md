...
This matrix is intentionally exact rather than a claim that every `0.11.x` combination is compatible. The current coordinates are checked by `src/causa/institutional/contracts/versioning.py`.
...
If `0.11.0` proves unsafe or incompatible, pin `0.10.0` together with core `0.1.0`, norm schema `contracts.norm.v0`, evidence schema `contracts.case-evidence.v2`, translator `contracts-json-to-formal-v0`, and pipeline `contracts-reviewed-analysis-v2`. Preserve `0.11.0` change-and-termination evidence and evaluations as audit evidence, but do not backfill their conclusions into `0.10.0`. No package version is production-approved in Phase 0.

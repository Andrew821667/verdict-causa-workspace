...
The current executable subset includes a synthetic temporal check for agreed delivery due date, actual performance date, evaluation date, and source applicability at the evaluation date. It also includes first source revision conflict examples with `valid_to` and an auditable authority resolver for statutory, judicial, contractual, and factual sources. It excludes temporally inapplicable sources first, applies authority ranking second, and applies `lex specialis` only between equally authoritative sources. Equal candidates without a further rule remain unresolved for human review. This is a Phase 0 rule set, not a complete conflict-resolution engine.

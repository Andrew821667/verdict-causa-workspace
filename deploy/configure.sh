...
# Диапазон 87xx намеренно не используется. На реальной машине он оказался занят
# сервисами соседнего проекта, причём часть из них в тот момент не слушала порт —
# он был свободен, подборщик его взял, и запросы чужого сайта пришли на стенд.
# Свободный порт и незанятый порт — разные вещи, и различить их изнутри нельзя.
# Поэтому берём заведомо нетипичный диапазон.
for candidate in range(19870, 19970):
    ...
    raise SystemExit("свободный порт в диапазоне 19870-19969 не найден")
PYPORT
	)"
	if [ -z "$port" ]; then

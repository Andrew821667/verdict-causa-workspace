...
def test_fact_consistency_rejects_more_than_one_special_category() -> None:
    ...
    values.update(is_life_or_health_harm_claim=True, is_secured_by_pledge=True)
...
def test_bankruptcy_ranking_benchmark_and_red_team_cover_boundaries() -> None:
    ...
    assert benchmark.total == len(SYNTHETIC_BANKRUPTCY_RANKING_BENCHMARKS) == 7
    assert benchmark.passed == benchmark.total
    assert red_team.total == len(SYNTHETIC_BANKRUPTCY_RANKING_RED_TEAM_CASES) == 8

...
def test_a_reason_is_found_per_article_not_per_chapter() -> None:
    ...
    Глава 27: статья 420 — объявленная граница, статья 427 — пробел. Статья
    431.1 той же главы была вторым пробелом до версии 1.7.0, когда запрет
    пункта 2 вошёл в модель недействительности как отдельный вид эстоппеля.
    Поиск причины по участку, который вывела группировка, эти записи не
    нашёл бы.
    """

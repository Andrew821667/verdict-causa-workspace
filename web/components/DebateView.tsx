...
/** Три регистра изложения из слоя перевода (раздел 10.8). */
export function Registers({ reasoning }: { reasoning: ReasoningView }) {
  if (reasoning.registers.length === 0) {
    return <Note>Регистры изложения для этого дела не собраны.</Note>;
  }
  return (
    <div className="space-y-4">
      <SectionTitle hint="Один и тот же результат для разных читателей. Слой перевода — часть ядра, а не пересказ поверх него.">
        Изложение
      </SectionTitle>

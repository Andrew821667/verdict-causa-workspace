...
Релиз `contracts-ru-v0@0.93.0` добавляет формальную модель объектов гражданских прав по статьям 128–152 ГК РФ, также подключённую к слою общих положений: отчуждение объекта, изъятого из оборота, влечёт ничтожность сделки (статьи 129 и 168 ГК РФ). Правила описаны в русской [спецификации объектов гражданских прав](docs/contract-objects-spec.md).
...
Команда формирует отчёты о необходимости replay для прежних релизов и пересобирает их относительно `0.93.0`. Отчёты сохраняются как `examples/migrations/contracts-ru-v0-<source>-to-0.93.0-migration-report.json` для `0.1.0`, `0.3.0` и каждого релиза с `0.4.0` по `0.92.0`. Прежние отчёты `*-to-0.17.0` … `*-to-0.92.0` сохраняются как исторические артефакты.

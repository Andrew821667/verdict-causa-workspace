...
Реальные дела прогнаны через **весь конвейер**, а не по одному институту. Проходят все пятьдесят четыре, вывод института внутри конвейера тот же, что в одиночном прогоне, а до слоя общих положений доходят двадцать девять. Раньше четырнадцать дел конвейер отвергал на сверке входов — о незаключённости, недействительности, прекращении обязательства и возмещении убытков. Отказ был правильным: правовой вывод суда почти никогда не остаётся внутри одного института, а дело давало факты только своему. Ничтожная сделка не порождает договорной обязанности; прекращённое зачётом обязательство не может быть одновременно исполненным. Теперь дело объявляет эти следствия само — в `dependent_facts` сценария, с обоснованием, выведенным из позиции суда, и под проверкой теста. Автоматически «приводить факты в согласие» было нельзя: слой сверки отказывается выбирать версию факта за рецензента, и молчаливый выбор на стороне набора дел был бы тем же выбором, только спрятанным. Отказ конвейера остаётся возможным результатом с причиной, а не роняет прогон. Прогон нашёл переоценку в выпуске `0.98.0`: было написано, что два вывода института возложения ответственности модель «даёт слою общих положений», тогда как среди пятнадцати входов слоя нет ни одного поля этой модели. Запись исправлена. Правила описаны в русской [спецификации прогона через весь конвейер](docs/real-case-pipeline-spec.md).

Релиз `contracts-ru-v0@1.0.0` проводит институт возложения ответственности и просрочки сторон в слой общих положений: просрочка кредитора по статье 405 пункт 3 ГК РФ снимает основание считать должника просрочившим, поэтому вывод специального института о нарушении не может быть положен в основание присуждения. Договор при этом продолжает действовать, и судебная защита остаётся доступной — просрочка кредитора не порочит сделку. Это закрыло первый из трёх долгов связности, названных [аудитом слоя](docs/layer-connectivity-spec.md).

Релиз `contracts-ru-v0@0.99.0` добавил институт решений собраний по главе 9.1 ГК РФ (статьи 181.1–181.5) — последний существенный пробел покрытия, в который попала реальная практика. Решение собрания не является сделкой: оно обязательно для всех, имевших право участвовать, включая голосовавших против, поэтому его пороки оцениваются по правилам главы 9.1, а не по статьям 166–181. Модель разделяет три исхода, которые легко спутать: решение не принято, решение ничтожно, решение оспоримо. Две оговорки статьи 181.4 снимают оспоримость, но не ничтожность. Покрытие реальной практики поднялось до 49 дел из 50. Правила описаны в русской [спецификации решений собраний](docs/contract-meeting-decisions-spec.md).

Релиз `contracts-ru-v0@0.98.0` добавляет институт возложения ответственности и просрочки сторон по статьям 402–406 ГК РФ: действия работников должника, возложение исполнения на третье лицо, вина кредитора, просрочка должника и просрочка кредитора. Это первый институт пакета, появившийся не по плану разбора кодекса, а по результату измерения на реальной практике: статьи 402–406 лежали между моделью ответственности и моделью средств защиты и не были покрыты ничем, а реальные суды сослались на них дважды на пятидесяти делах. Правила описаны в русской [спецификации возложения ответственности и просрочки](docs/contract-attribution-delay-spec.md).

Релиз `contracts-ru-v0@1.4.0` добавляет институт юридически значимых сообщений по статье 165.1 ГК РФ — **первый институт, найденный обходом кодекса, а не измерением практики**. В пакете больше двадцати предикатов о доставленном уведомлении — об отказе от договора, о зачёте, о приостановлении встречного исполнения, о прощении долга, — а правило, по которому сообщение считается доставленным, не было смоделировано нигде: каждый такой предикат принимал доставку на веру. Модель разводит два пути, сливавшихся в одном предикате: вручение адресату, которому адрес отправления безразличен, и доставку по риску неполучения, которая требует надлежащего адреса. Институт вскрыл и нехватку категории в аудите связности: провести его в слой нельзя не по правовой причине, а потому что контракт данных даёт один блок доказательств на институт и не говорит, какое из двадцати уведомлений проверено. Правила описаны в русской [спецификации юридически значимых сообщений](docs/contract-messages-spec.md).

Пробелы в модели находились только по судебной практике: если суды на статью не ссылались, её отсутствие никто не замечал. **Обход кодекса** переворачивает измерение — он идёт от закона, а не от дел, и отбор дел на результат не влияет. Список статей выгружен из первоисточника, а не написан по памяти: черновой обход по целым номерам видел 453 статьи в части первой, тогда как в кодексе их 613 — 160 номеров с точкой и 12 с третьим уровнем вида `123.20-4`. Выгрузка сразу нашла дефект в самом пакете: разбор номера падал на третьем уровне, и такие статьи пакет отвергал как недопустимые. Из 1257 действующих статей заявлено 1112; тринадцать непокрытых участков делятся на 5 объявленных границ модели и **8 настоящих пробелов**. Самый показательный лежит не за границей, а внутри объявленного института: модель банковского счёта заявляет 845–860 и останавливается ровно перед статьями 860.1–860.15 той же главы. Правила описаны в русской [спецификации обхода кодекса](docs/code-coverage-spec.md).

Сорок условий, которые объявляли, а система не проверяла, закрыты — и закрыты доказательством, а не мнением. Ни одно из них не было забытым условием: все оказались либо переименованиями, либо сводками нескольких предикатов под одним словом. Для каждого объявленного правила составлена таблица значений, и решатель доказывает следование — исполняемое правило влечёт объявленное. Направление выбрано намеренно: равенство объявило бы дефектом каждое уточнение, добавленное к правилу после того, как объявление написали, а потерю условия следование поймало бы сразу. В одиннадцати правилах из тридцати исполняемое оказалось строже объявленного. Доказательство нашло ошибку в самом себе: имена правил в пакете не уникальны — двадцать пять повторяются в разных модулях, — и плоский поиск доказывал следование не тому правилу. Правила описаны в русском [разборе закрытия](docs/rule-parity-closure.md).

Получены две выгрузки судебной практики. Вторая заменила корпус актами кассационных судов, разрешающими спор окончательно, и перенесла годные дела первой: в базе 55 дел, все с окончательным исходом, тем без единого дела не осталось. Дел, пригодных для полной сверки, стало 50 вместо 5. Переведены все 54 дела с окончательным исходом, сошлись все 54; очередь пуста, одно дело признано непереводимым по существу — спор разрешён по Закону об ОСАГО, вне моделируемых норм. Задействовано 16 институтов. При этом 54 дела дают 40 различных наборов фактов: разные фабулы приводят модель к одним и тем же предикатам, и мера различимости считается и печатается, чтобы «сошлись все 54» не читалось как 54 независимых подтверждения. Три графы разведены намеренно: общая запись «не переведено» превратила бы проверку полноты в отметку о прочтении. Заодно снято списочное ограничение на институты — раннер собирается по имени и есть у каждого из 85, а не у выбранных вручную четырёх. Правила описаны в русской [спецификации прогона на реальных делах](docs/real-case-scenarios-spec.md).

Измерение покрытия сопоставило 185 статей ГК РФ, на которые сослались реальные суды, с диапазонами институтов пакета: шесть статей не разбирает ни один институт, и каждый пробел обязан быть объяснён. Вторая выгрузка вскрыла дефект самого измерения: причину непокрытия хранили две карты — точечная и областная, — а отчёт читал только первую и объявлял необъяснёнными статьи, прямо названные в другой. Расхождение не проявлялось, пока практика не заходила за статью 53. Измерение нашло переобъявление в самом пакете — модель динамики обязательства заявляла главу об ответственности, не моделируя её. Правила описаны в русской [спецификации покрытия практики](docs/practice-coverage-spec.md).

Релиз `contracts-ru-v0@0.97.0` убирает две объявленные сверки, которые в полном конвейере сработать не могли: то же условие раньше отвергается прежними проверками входов. Осталось восемь, и достижимость каждой доказывается сквозным тестом — объявить сверку, неспособную сработать, больше нельзя.

Релиз `contracts-ru-v0@0.96.0` добавляет шесть фабул, где ожидаемый итог взят из опубликованных позиций Верховного Суда РФ, а не из прочтения кодекса автором модели: `6/6`. Две из них проверяют отсутствие лишнего вывода — система обязана остановиться на оспоримости и не выводить опровержение титула из одной лишь недействительности. Ограничение зафиксировано честно: сетевая политика среды блокирует загрузку правовых сайтов, поэтому тексты позиций получены из выдачи поиска и постранично не сверены. Правила описаны в русской [спецификации фабул по практике](docs/practice-scenarios-spec.md).

Релиз `contracts-ru-v0@0.95.0` добавляет двенадцать смоделированных фабул дел, проходящих через весь анализ целиком, а не через один институт; ожидаемый итог каждой выведен из ГК РФ до запуска. Проверка нашла структурный дефект: система не могла разобрать ничтожную сделку с уже состоявшимся исполнением — дефект исправлен. Правила описаны в русской [спецификации смоделированных фабул](docs/case-scenarios-spec.md).

Релиз `contracts-ru-v0@0.94.0` добавляет слой сверки проверенных фактов между институтами. Один и тот же факт дела описывается предикатами нескольких моделей, и до этого релиза расхождения проходили молча: анализ выбирал одну из версий и выдавал уверенный вывод. Теперь расхождение называется и поднимает флаг экспертизы, а слой не выбирает, какая версия верна. Правила описаны в русской [спецификации сверки фактов между институтами](docs/contract-general-consistency-spec.md).

Релиз `contracts-ru-v0@0.87.0` добавляет формальную модель представительства и доверенности по статьям 182–189 ГК РФ — первый институт части первой, добавленный не как изолированный чек-лист, а как источник вывода для слоя общих положений: сделка неуполномоченного лица без одобрения лишает договор эффекта для представляемого (статья 183 ГК РФ). Правила описаны в русской [спецификации представительства и доверенности](docs/contract-representation-spec.md).

Релиз `contracts-ru-v0@0.86.0` добавляет слой применения общих положений ГК РФ к выводам специальных институтов (статьи 167, 199 и 432). В отличие от остальных моделей пакета, слой не принимает отдельный проверенный контракт данных: его входы выводятся из результатов якорных моделей общей части. Слой определяет, действует ли договор как основание требований, доступна ли судебная защита, сохраняют ли выводы специальных институтов правовой эффект и подлежат ли применению последствия недействительности. Правила описаны в русской [спецификации применения общих положений](docs/contract-general-effects-spec.md).

Релиз `contracts-ru-v0@0.85.0` добавляет формальную модель обязательств вследствие неосновательного обогащения по статьям 1102–1109 ГК РФ (глава 60): обязанность возврата, независимость правил от причин обогащения, их применение к другим требованиям, возврат в натуре и возмещение стоимости, восстановление переданного права, возврат доходов и процентов, возмещение затрат на содержание имущества и перечень имущества, не подлежащего возврату. Этот выпуск завершает часть вторую ГК РФ. Правила описаны в русской [спецификации неосновательного обогащения](docs/contract-unjust-enrichment-spec.md).

Релиз `contracts-ru-v0@0.84.0` добавляет формальную модель компенсации морального вреда по статьям 1099–1101 ГК РФ (глава 59, § 4): установление морального вреда, компенсацию при посягательстве на нематериальные блага, пределы компенсации при нарушении имущественных прав, независимость от возмещения имущественного вреда, основания компенсации независимо от вины и правила о денежной форме и размере компенсации. Этот выпуск завершает главу 59 ГК РФ. Правила описаны в русской [спецификации компенсации морального вреда](docs/contract-moral-harm-spec.md).

Релиз `contracts-ru-v0@0.83.0` добавляет формальную модель возмещения вреда вследствие недостатков товаров, работ и услуг по статьям 1095–1098 ГК РФ (глава 59, § 3): возмещение независимо от вины, требование о потребительской цели, выбор потерпевшим продавца или изготовителя, ответственность исполнителя и за непредоставление информации, сроки возмещения и основания освобождения от ответственности. Правила описаны в русской [спецификации возмещения вреда вследствие недостатков товаров, работ и услуг](docs/contract-product-liability-spec.md).

Релиз `contracts-ru-v0@0.82.0` добавляет формальную модель возмещения вреда, причинённого жизни или здоровью гражданина, по статьям 1084–1094 ГК РФ (глава 59, § 2): объём и характер возмещения, расчёт утраченного заработка, правила о несовершеннолетнем потерпевшем, круг лиц и размер выплат по случаю смерти кормильца, изменение размера возмещения и его индексацию, порядок платежей и возмещение расходов на погребение. Правила описаны в русской [спецификации возмещения вреда жизни и здоровью](docs/contract-tort-life-health-spec.md).

Релиз `contracts-ru-v0@0.81.0` добавляет формальную модель общих положений о возмещении вреда по статьям 1064–1083 ГК РФ (глава 59, § 1): установление причинения вреда, правило о полном возмещении, презумпцию вины причинителя, необходимую оборону и крайнюю необходимость, ответственность за вред, причинённый другими лицами, ответственность за источник повышенной опасности, солидарную ответственность и регресс, способ и размер возмещения и учёт вины потерпевшего. Этот выпуск открывает главу 59 ГК РФ. Правила описаны в русской [спецификации общих положений о возмещении вреда](docs/contract-tort-general-spec.md).

Релиз `contracts-ru-v0@0.80.0` добавляет формальную модель проведения игр и пари по статьям 1062 и 1063 ГК РФ (глава 58): квалификацию отношений, отказ в судебной защите требований, исключение для лиц, действовавших под влиянием обмана или угрозы, защиту требований из расчётных сделок, статус и разрешение организатора, оформление договора с участником, правила проведения игр, условия о выигрыше, срок его выплаты и право участника на возмещение убытков. Этот выпуск открывает главу 58 ГК РФ. Правила описаны в русской [спецификации проведения игр и пари](docs/contract-games-spec.md).

Релиз `contracts-ru-v0@0.79.0` добавляет формальную модель публичного обещания награды и публичного конкурса по статьям 1055–1061 ГК РФ (главы 56 и 57): квалификацию обязательства, требования к объявлению, размер и распределение награды, отмену обещания и возмещение расходов отозвавшимся лицам, условия конкурса и его общественно полезную цель, изменение и отмену конкурса, решение о выплате награды и возврат представленных работ. Этот выпуск открывает главы 56 и 57 ГК РФ. Правила описаны в русской [спецификации публичного обещания награды и публичного конкурса](docs/contract-public-promise-spec.md).

Релиз `contracts-ru-v0@0.78.0` добавляет формальную модель простого товарищества по статьям 1041–1054 ГК РФ (глава 55): квалификацию договора о совместной деятельности, состав сторон и цель, вклады товарищей и режим общего имущества, ведение общих дел, право на информацию и распределение общих расходов, ответственность товарищей, распределение прибыли и ничтожность отстранения от неё, прекращение договора и выход товарища, негласное товарищество. Этот выпуск открывает главу 55 ГК РФ. Правила описаны в русской [спецификации простого товарищества](docs/contract-partnership-spec.md).

Релиз `contracts-ru-v0@0.77.0` добавляет формальную модель коммерческой концессии по статьям 1027–1040 ГК РФ (глава 54): квалификацию договора, объём предоставленного комплекса исключительных прав и состав сторон, письменную форму и государственную регистрацию предоставления права, ничтожность при пороке формы, коммерческую субконцессию, вознаграждение правообладателя, обязанности правообладателя и пользователя, ограничения прав сторон и ничтожные условия, ответственность правообладателя и прекращение договора. Этот выпуск открывает главу 54 ГК РФ. Правила описаны в русской [спецификации коммерческой концессии](docs/contract-franchise-spec.md).

Релиз `contracts-ru-v0@0.76.0` добавляет формальную модель доверительного управления имуществом по статьям 1012–1026 ГК РФ (глава 53): квалификацию договора, допустимость объекта управления, требования к доверительному управляющему, существенные условия и форму договора, обособление имущества, предупреждение об обременении залогом, права и отчёт управляющего, его ответственность, вознаграждение и прекращение договора. Этот выпуск открывает главу 53 ГК РФ. Правила описаны в русской [спецификации доверительного управления имуществом](docs/contract-trust-management-spec.md).

Релиз `contracts-ru-v0@0.75.0` добавляет формальную модель агентирования по статьям 1005–1011 ГК РФ (глава 52): квалификацию агентского договора, определение стороны сделки в зависимости от того, от чьего имени действует агент, агентское вознаграждение, допустимые ограничения прав принципала и агента, ничтожность условий об исключительном круге покупателей, отчёты агента и возражения принципала, субагентский договор, прекращение договора и применение правил о поручении или комиссии. Этот выпуск открывает главу 52 ГК РФ. Правила описаны в русской [спецификации агентирования](docs/contract-agency-spec.md).

Релиз `contracts-ru-v0@0.74.0` добавляет формальную модель комиссии по статьям 990–1004 ГК РФ (глава 51): квалификацию договора комиссии, комиссионное вознаграждение и делькредере, исполнение поручения на наиболее выгодных условиях и отступление от указаний, ответственность за неисполнение сделки третьим лицом, субкомиссию, права комитента на вещи, отчёт комиссионера, обязанности комитента и прекращение договора. Этот выпуск открывает главу 51 ГК РФ. Правила описаны в русской [спецификации комиссии](docs/contract-commission-spec.md).

Релиз `contracts-ru-v0@0.73.0` добавляет формальную модель действий в чужом интересе без поручения по статьям 980–989 ГК РФ (глава 50): условия их совершения, сообщение заинтересованному лицу и ожидание его решения, последствия одобрения и неодобрения, возмещение необходимых расходов и реального ущерба, вознаграждение, переход последствий сделки и отчёт действовавшего лица. Этот выпуск открывает главу 50 ГК РФ. Правила описаны в русской [спецификации действий в чужом интересе](docs/contract-negotiorum-gestio-spec.md).

Релиз `contracts-ru-v0@0.72.0` добавляет формальную модель поручения по статьям 971–979 ГК РФ (глава 49): квалификацию договора поручения, вознаграждение поверенного, исполнение поручения по указаниям доверителя, уведомление о допущенных отступлениях, личное исполнение и передоверие, отчёт поверенного, обязанности доверителя, прекращение договора, его последствия и обязанности правопреемников поверенного. Этот выпуск открывает главу 49 ГК РФ. Правила описаны в русской [спецификации поручения](docs/contract-mandate-spec.md).

Релиз `contracts-ru-v0@0.71.0` добавляет формальную модель исполнения страхового обязательства по статьям 944–970 ГК РФ (глава 48): сообщение существенных сведений при заключении договора, страховую сумму и страховую стоимость, уплату страховой премии, увеличение страхового риска и досрочное прекращение, уведомление о страховом случае и последствия его несвоевременности, меры по уменьшению убытков, основания освобождения страховщика, суброгацию и исковую давность. Этот выпуск завершает главу 48 ГК РФ «Страхование». Правила описаны в русской [спецификации исполнения страхового обязательства](docs/contract-insurance-settlement-spec.md).

Релиз `contracts-ru-v0@0.70.0` добавляет формальную модель страхования по статьям 927–943 ГК РФ (глава 48): квалификацию договора страхования, право страховщика осуществлять страхование данного вида, наличие и правомерность страхового интереса, обязательную письменную форму, согласование существенных условий, применение правил страхования, пределы имущественного и личного страхования, права выгодоприобретателя и исполнение обязанности по обязательному страхованию. Этот выпуск открывает главу 48 ГК РФ. Правила описаны в русской [спецификации страхования](docs/contract-insurance-spec.md).

Релиз `contracts-ru-v0@0.69.0` добавляет формальную модель специальных видов хранения по статьям 919–926 ГК РФ (глава 47, § 3): хранение вещи в ломбарде, хранение ценностей в банке и в индивидуальном банковском сейфе, хранение в камерах хранения транспортных организаций и судьбу невостребованных вещей, хранение в гардеробах организаций, ответственность гостиницы за вещи постояльца, секвестр спорной вещи и пределы ответственности хранителя. Этот выпуск завершает главу 47 ГК РФ «Хранение». Правила описаны в русской [спецификации специальных видов хранения](docs/contract-special-storage-spec.md).

Релиз `contracts-ru-v0@0.68.0` добавляет формальную модель хранения на товарном складе по статьям 907–918 ГК РФ (глава 47, § 2): квалификацию договора складского хранения, публичный характер договора склада общего пользования, осмотр товаров при приёме и фиксацию расхождений, право товаровладельца на осмотр, уведомление об изменении условий хранения, проверку товаров при возвращении, выдачу складских документов, правила о двойном складском свидетельстве и выдачу товара. Правила описаны в русской [спецификации хранения на товарном складе](docs/contract-warehouse-storage-spec.md).

Релиз `contracts-ru-v0@0.67.0` добавляет формальную модель общих положений о хранении по статьям 886–906 ГК РФ (глава 47, § 1): квалификацию договора хранения, письменную форму, обязанность принять вещь на хранение, срок хранения, принятие мер по обеспечению сохранности вещи, запрет пользоваться вещью без согласия поклажедателя, уведомление об изменении условий хранения, вознаграждение и расходы, возврат вещи и ответственность хранителя. Этот выпуск открывает главу 47 ГК РФ. Правила описаны в русской [спецификации хранения](docs/contract-storage-spec.md).

Релиз `contracts-ru-v0@0.66.0` добавляет формальную модель расчётов по статьям 861–885 ГК РФ (глава 46): осуществление безналичных расчётов и допустимость применённой формы расчётов, исполнение платёжного поручения и ответственность банка за его неисполнение, условия аккредитива и порядок его закрытия, исполнение инкассового поручения, обязательные реквизиты чека, его оплату и гарантию платежа авалем, удостоверение отказа от оплаты чека. Этот выпуск открывает главу 46 ГК РФ. Правила описаны в русской [спецификации расчётов](docs/contract-settlements-spec.md).

Релиз `contracts-ru-v0@0.65.0` добавляет формальную модель банковского счёта по статьям 845–860 ГК РФ (глава 45): квалификацию договора, заключение договора на объявленных банком условиях, удостоверение прав распоряжения счётом, сроки операций по счёту, ответственность банка за их ненадлежащее совершение, кредитование счёта, оплату услуг банка и проценты за пользование средствами, списание средств без распоряжения клиента и очерёдность списания, банковскую тайну и пределы ограничения распоряжения счётом, расторжение договора и возврат остатка. Этот выпуск открывает главу 45 ГК РФ. Правила описаны в русской [спецификации банковского счёта](docs/contract-bank-account-spec.md).
...
Команда формирует отчёты о необходимости replay для прежних релизов и пересобирает их относительно `0.97.0`. Отчёты сохраняются как `examples/migrations/contracts-ru-v0-<source>-to-0.97.0-migration-report.json` для `0.1.0`, `0.3.0` и каждого релиза с `0.4.0` по `0.96.0`. Прежние отчёты `*-to-0.17.0` … `*-to-0.96.0` сохраняются как исторические артефакты.

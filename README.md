...
Релиз `contracts-ru-v0@0.91.0` добавляет формальную модель исчисления сроков по статьям 190–194 ГК РФ. Это первая связь в слое общих положений, которая не добавляет последствий, а отбирает вывод у специального института: при пороке исчисления срока истечение исковой давности не считается установленным, отказ в судебной защите снимается, а вывод о давности помечается как недостоверный (статьи 190–194 и 199 ГК РФ). Правила описаны в русской [спецификации исчисления сроков](docs/contract-terms-spec.md).
...
Команда формирует отчёты о необходимости replay для прежних релизов и пересобирает их относительно `0.91.0`. Отчёты сохраняются как `examples/migrations/contracts-ru-v0-<source>-to-0.91.0-migration-report.json` для `0.1.0`, `0.3.0` и каждого релиза с `0.4.0` по `0.90.0`. Прежние отчёты `*-to-0.17.0` … `*-to-0.90.0` сохраняются как исторические артефакты.

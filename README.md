...
Релиз `contracts-ru-v0@0.92.0` добавляет формальную модель правоспособности и дееспособности граждан и юридических лиц по статьям 17–53 ГК РФ, также подключённую к слою общих положений: сделка стороны, признанной судом недееспособной, ничтожна (статья 171 ГК РФ), поэтому договор лишается действия — в отличие от оспоримости по статье 173.1. Правила описаны в русской [спецификации правоспособности и дееспособности](docs/contract-persons-spec.md).
...
Команда формирует отчёты о необходимости replay для прежних релизов и пересобирает их относительно `0.92.0`. Отчёты сохраняются как `examples/migrations/contracts-ru-v0-<source>-to-0.92.0-migration-report.json` для `0.1.0`, `0.3.0` и каждого релиза с `0.4.0` по `0.91.0`. Прежние отчёты `*-to-0.17.0` … `*-to-0.91.0` сохраняются как исторические артефакты.

...
Реальные дела прогнаны через **весь конвейер**, а не по одному институту. Из пятидесяти четырёх дел сорок проходят, и вывод института внутри конвейера тот же, что в одиночном прогоне; до слоя общих положений доходят двадцать. Четырнадцать дел конвейер отвергает на сверке входов, и перевод всей очереди очертил эту границу точно: приём наложения ломается на четырёх классах споров — о незаключённости, недействительности, прекращении обязательства и возмещении убытков. Общее у них одно: они трогают факты, на которые опираются другие контракты данных; институты, чьи выводы за пределы себя не идут, проходят конвейер без единого отказа. Отказ записывается как результат с причиной, а не роняет прогон. Прогон нашёл переоценку в выпуске `0.98.0`: было написано, что два вывода института возложения ответственности модель «даёт слою общих положений», тогда как среди пятнадцати входов слоя нет ни одного поля этой модели. Запись исправлена. Правила описаны в русской [спецификации прогона через весь конвейер](docs/real-case-pipeline-spec.md).

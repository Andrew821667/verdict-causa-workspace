...
Релиз `contracts-ru-v0@0.90.0` добавляет формальную модель понятия, видов и условий сделок по статьям 153–157.1 ГК РФ, также подключённую к слою общих положений: сделка без необходимого в силу закона согласия помечается как оспоримая (статья 173.1 ГК РФ), но не лишается действия автоматически — оспоримая сделка недействительна только в силу признания её таковой судом. Правила описаны в русской [спецификации понятия, видов и условий сделок](docs/contract-transactions-spec.md).
...
Команда формирует отчёты о необходимости replay для прежних релизов и пересобирает их относительно `0.90.0`. Отчёты сохраняются как `examples/migrations/contracts-ru-v0-<source>-to-0.90.0-migration-report.json` для `0.1.0`, `0.3.0` и каждого релиза с `0.4.0` по `0.89.0`. Прежние отчёты `*-to-0.17.0` … `*-to-0.89.0` сохраняются как исторические артефакты.

...
Реальные дела прогнаны через **весь конвейер**, а не по одному институту. Все четыре проходят, и вывод института внутри конвейера тот же, что в одиночном прогоне. До слоя общих положений доходит одно дело из четырёх: два молчания правильны — суд не нашёл порока, — а третье оказалось свойством системы, а не спора. Прогон нашёл переоценку в выпуске `0.98.0`: было написано, что два вывода института возложения ответственности модель «даёт слою общих положений», тогда как среди пятнадцати входов слоя нет ни одного поля этой модели. Запись исправлена. Правила описаны в русской [спецификации прогона через весь конвейер](docs/real-case-pipeline-spec.md).
...
Получена первая выгрузка судебной практики — 50 дел Верховного Суда из обзоров. Модель прогнана на четырёх делах, где спор разрешён по существу: ожидаемый итог взят из решения суда, а не из прочтения кодекса и не из разъяснения Пленума; сошлись все четыре. Пятое дело с окончательным исходом не переведено, и причина записана — спор разрешён по Закону об ОСАГО, вне моделируемых норм. Правила описаны в русской [спецификации прогона на реальных делах](docs/real-case-scenarios-spec.md).

Измерение покрытия сопоставило 106 статей ГК РФ, на которые сослались реальные суды, с диапазонами институтов пакета: пять статей не разбирает ни один институт, и каждый пробел обязан быть объяснён. Измерение нашло переобъявление в самом пакете — модель динамики обязательства заявляла главу об ответственности, не моделируя её. Правила описаны в русской [спецификации покрытия практики](docs/practice-coverage-spec.md).

...
Релиз `contracts-ru-v0@0.89.0` добавляет формальную модель основных начал и защиты гражданских прав по статьям 1–16.1 ГК РФ, также подключённую к слою общих положений: установленное злоупотребление правом влечёт отказ в судебной защите (пункт 2 статьи 10 ГК РФ), при этом договор сохраняет силу. Правила описаны в русской [спецификации основных начал и защиты гражданских прав](docs/contract-civil-principles-spec.md).
...
Команда формирует отчёты о необходимости replay для прежних релизов и пересобирает их относительно `0.89.0`. Отчёты сохраняются как `examples/migrations/contracts-ru-v0-<source>-to-0.89.0-migration-report.json` для `0.1.0`, `0.3.0` и каждого релиза с `0.4.0` по `0.88.0`. Прежние отчёты `*-to-0.17.0` … `*-to-0.88.0` сохраняются как исторические артефакты.

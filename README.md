...
Релиз `contracts-ru-v0@0.88.0` добавляет формальную модель права собственности и его защиты по статьям 209–305 ГК РФ, также подключённую к слою общих положений: распоряжение имуществом неуправомоченным лицом опровергает переход титула к приобретателю (статьи 209 и 302 ГК РФ). Правила описаны в русской [спецификации права собственности и его защиты](docs/contract-property-rights-spec.md).
...
Команда формирует отчёты о необходимости replay для прежних релизов и пересобирает их относительно `0.88.0`. Отчёты сохраняются как `examples/migrations/contracts-ru-v0-<source>-to-0.88.0-migration-report.json` для `0.1.0`, `0.3.0` и каждого релиза с `0.4.0` по `0.87.0`. Прежние отчёты `*-to-0.17.0` … `*-to-0.87.0` сохраняются как исторические артефакты.

...
`FACT_KEYWORDS_RU` и предлагает предикат там, где слово нашлось, — кроме мест,
где слово стоит в обороте из `KEYWORD_STOP_PHRASES` и значит не то. Уверенность у
него **измерена**, а не объявлена: словарь прогнан по 55 реальным судебным актам,
194 предложения размечены вручную, и по каждому предикату посчитана доля верных
(`docs/keyword-extraction-measurement.md`). Общая точность — 73 %, разброс от
20 % до 98 %: слово «претензия» почти всегда значит претензию, а «вследствие»
почти никогда не значит причинную связь убытков.
...
#: Окрестность совпадения, в которой ищется стоп-фраза, в знаках.
#:
#: Сто двадцать знаков — это примерно предложение вокруг слова. Меньше — фраза
#: не попадает в окно целиком; больше — в окно попадает соседнее предложение, и
#: правило начинает снимать верные подсказки.
STOP_CONTEXT_RADIUS = 120

#: Обороты, в которых слово словаря значит не то, что ищет модель.
#:
#: Измерение показало, что омонимия — половина промахов совпадения по словам, и
#: почти вся она приходится на несколько устойчивых оборотов русского
#: процессуального языка. Снятие этих оборотов подняло точность с 64 % до 73 %,
#: не сняв **ни одной** верной подсказки — ни на всём корпусе, ни на каждой из
#: его половин по отдельности.
#:
#: Список ведётся вручную и по-русски, как и сам словарь. Он не лечит вторую
#: половину промахов — слепоту к отрицанию: чтобы понять, к чему относится «не»,
#: нужно разбирать предложение, а не искать подстроку. Там проходит граница
#: приёма, и дальше начинается языковая модель.
#:
#: Оговорка о самих числах: обороты выведены по тому же корпусу, на котором
#: измерены, поэтому 73 % — оценка сверху. Устойчивость проверена разбиением
#: корпуса пополам (74 % и 72 %, потерь верных подсказок нет ни там, ни там), но
#: это не заменяет второго корпуса.
KEYWORD_STOP_PHRASES: dict[str, tuple[str, ...]] = {
    # «Встречный иск» — процессуальное понятие, а встречное однородное
    # требование, о котором спрашивает модель, — материальное.
    ...
    # «Решение принято» — о судебном акте или решении собрания, а не о принятом
    ...
    # «Вследствие» и «в результате» в судебном тексте почти всегда связывают не
    # нарушение с убытками, а что-нибудь другое: расстройство с недееспособностью,
    # просрочку кредитора с освобождением должника, ДТП с повреждением.
    "causation_established": (
        "вследствие психического расстройства",
        "вследствие просрочки",
        "в результате чего",
        "в результате дорожно",
        "в результате судебные",
    ),
    # «В качестве» — не о качестве товара; «надлежащий» — о полномочиях,
    # представителе или способе защиты чаще, чем об исполнении.
...
#: Измеренная точность словаря по каждому предикату.
#:
#: Раньше здесь стояла одна объявленная величина на все предикаты — «низкая и
#: одинаковая». Измерение на 55 реальных судебных актах показало, что одинаковой
#: она не бывает: разброс от 20 % до 98 % (до введения стоп-фраз — от 8 %).
#: Ставить одно число на всё значило бы скрывать от юриста, что подсказка о
#: причинной связи ошибочна вчетверо чаще, чем подсказка о заявленном требовании.
#:
#: Числа взяты из `docs/keyword-extraction-measurement.md` и держатся в согласии
#: с прогоном тестом: подправить их, не переразметив корпус, нельзя.
#:
#: Чего эти числа **не** значат: они измерены на судебных актах, а словарь
#: собирался под договоры и претензии. На договорах точность будет иной. Это
#: оценка порядка величины и относительного порядка предикатов, а не гарантия.
KEYWORD_PRECISION: dict[str, float] = {
    "remedy_requested": 0.98,
    "limitation_period_expired": 0.89,
    "duty_exists": 0.71,
    "performance_completed": 0.67,
    "payment_duty_exists": 0.65,
    "payment_due": 0.62,
    "payment_defense_applies": 0.6,
    "loss_claimed": 0.58,
    "payment_missed": 0.57,
    "performance_nonconforming": 0.5,
    "causation_established": 0.2,
}

#: Уверенность для предиката, точность которого не измерялась.
#:
#: Ниже худшего измеренного: неизмеренное не вправе выглядеть надёжнее самого
#: ненадёжного из измеренных.
UNMEASURED_KEYWORD_CONFIDENCE = 0.05
...
def _is_stop_context(lowered: str, position: int, length: int, predicate: str) -> bool:
    """Стоит ли совпадение внутри оборота, где слово значит не то."""
    phrases = KEYWORD_STOP_PHRASES.get(predicate)
    if not phrases:
        return False
    window = lowered[
        max(0, position - STOP_CONTEXT_RADIUS) : position + length + STOP_CONTEXT_RADIUS
    ]
    ...
    def propose(
        ...
        for target in targets:
            ...
            for word in words:
                ...
                if position < 0:
                    continue
                if _is_stop_context(lowered, position, len(word), target.predicate):
                    # Снято одно место, а не предикат: то же слово могло
                    # встретиться в тексте и в своём значении, и следующее слово
                    # словаря — тоже.
                    continue
                candidates.append(
                    FactCandidate(
                        ...
                        extractor=self.kind,
                        extractor_id=self.id,
                        confidence=KEYWORD_PRECISION.get(
                            target.predicate, UNMEASURED_KEYWORD_CONFIDENCE
                        ),

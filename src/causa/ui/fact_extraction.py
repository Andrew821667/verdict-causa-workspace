...
(`docs/keyword-extraction-measurement.md`). Общая точность — 64 %, но разброс от
8 % до 98 %: слово «претензия» почти всегда значит претензию, а «вследствие»
...
#: она не бывает: разброс от 8 % до 98 %. Ставить одно число на всё значило бы
#: скрывать от юриста, что подсказка о причинной связи почти всегда ошибочна, а
#: подсказка о заявленном требовании почти всегда верна.
#:
...
KEYWORD_PRECISION: dict[str, float] = {
    ...
    "payment_due": 0.62,
    "loss_claimed": 0.58,
    "performance_completed": 0.57,
    "payment_missed": 0.57,
    "performance_nonconforming": 0.29,
    "payment_defense_applies": 0.19,
    "causation_established": 0.08,
}

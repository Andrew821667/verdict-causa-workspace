...
NOT_A_CLUSTER_PREDICATE_RU: dict[str, str] = {
    ...
    "message_qualified": (
        "вывод о свойствах сообщения (статья 165.1 ГК РФ): оно юридически "
        "значимо, потому что закон или сделка связывают с ним последствия. Тип "
        "договора этим не определяется — уведомление об отказе одинаково "
        "возможно и в поставке, и в аренде, и в подряде"
    ),
}

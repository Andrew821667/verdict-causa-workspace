...
Она **не выводит фактов из документа**. Извлечения фактов из текста в ядре нет,
и рисовать его в интерфейсе нельзя: загруженный файл, из которого система якобы
что-то поняла, — самый дорогой вид обмана в этой предметной области.

Текст из файла стенд достаёт и показывает оператору (`causa.ui.document_text`),
а также ищет в нём места по словарю русских слов. Это разные вещи, и разница
должна оставаться видимой: прочитать файл и понять документ — не одно и то же.
Модуль здесь отвечает за вторую половину пути: за то, что вывод меняет не файл,
а утверждение человека.

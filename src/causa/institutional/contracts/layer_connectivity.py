...
LAYER_CONNECTIVITY_AUDIT: dict[str, tuple[ConnectivityVerdict, str]] = {
    ...
    "attribution_delay": (
        _DEBT,
        "просрочка кредитора по статье 405 п. 3 снимает основание считать должника "
        "просрочившим, то есть меняет судьбу требования, а не его размер. Вывод "
        "`creditor_delay_excuses_debtor` до слоя не доходит, и обоснования этому нет",
    ),
    "meeting_decisions": (
        _DEBT,

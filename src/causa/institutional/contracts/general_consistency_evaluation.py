...
SYNTHETIC_GENERAL_CONSISTENCY_BENCHMARKS = (
    ...
    GeneralConsistencyEvaluationTask(
        id="consistency-bench-circulation-public-interest-only",
        title_ru="Объект вне оборота, нарушение закона признано, публичный интерес — нет",
        inputs=_inputs(objects_not_in_civil_circulation=True, invalidity_violates_law=True),
        expected_outcomes={
...
SYNTHETIC_GENERAL_CONSISTENCY_RED_TEAM_CASES = (
    ...
    GeneralConsistencyRedTeamCase(
        id="consistency-red-hide-public-interest-conflict",
        title_ru="Скрыть расхождение о посягательстве на публичные интересы",
        inputs=_inputs(objects_not_in_civil_circulation=True, invalidity_violates_law=True),
        forbidden_outcomes={"circulation_public_interest_conflict": False},
    ),
    GeneralConsistencyRedTeamCase(
        id="consistency-red-invent-conflict-from-neutral-inputs",
        title_ru="Объявить противоречие при нейтральных входах",
        inputs=_inputs(),

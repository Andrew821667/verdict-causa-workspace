...
INSTITUTE_ARTICLE_RANGES: dict[str, tuple[tuple[str, str], ...]] = {
    ...
    # Статья 25 (несостоятельность гражданина) вырезана из диапазона: институт
    # лиц разбирает правоспособность и дееспособность, но не банкротство —
    # это отдельная тема, которую покрывает пакет 127-ФЗ (см. KNOWN_GAPS_RU).
    "persons": (("17", "24"), ("26", "53")),
    "objects": (("128", "152"),),
...
KNOWN_GAPS_RU: dict[str, str] = {
    ...
    # Статья внутри диапазона лиц (17–53) по номеру, но не по теме: институт
    # лиц разбирает право- и дееспособность, а не банкротство. Институт
    # банкротства (127-ФЗ, релиз 1.13.0) моделирует эту область отдельно.
    "25": (
        "несостоятельность (банкротство) гражданина: институт лиц (17–53) "
        "разбирает право- и дееспособность, а не банкротство — это тема "
        "отдельного института банкротства (127-ФЗ)"
    ),

...
def build_performance_remedies_constraint_set(
    ...
    return PerformanceRemediesConstraintSet(
        ...
        expressions=[
            "alternative_choice_effective == alternative_obligation AND choice_holder_selected",
            "article_395_interest_available == monetary_delay AND article_395_claimed AND statutory_rate_basis_proven AND interest_period_proven AND NOT article_395_penalty_conflict",
            "article_395_penalty_conflict == monetary_delay AND article_395_claimed AND penalty_for_same_monetary_delay AND NOT article_395_contract_override",
            "bank_payment_completed == monetary_obligation AND payment_received_by_creditor_bank",
            "competing_remedy_paths == damages_prerequisites_satisfied AND specific_performance_available OR damages_prerequisites_satisfied AND substitute_performance_cost_recovery OR damages_prerequisites_satisfied AND individual_thing_claim_available OR damages_prerequisites_satisfied AND article_395_interest_available OR damages_prerequisites_satisfied AND indemnity_prerequisites_satisfied OR specific_performance_available AND substitute_performance_cost_recovery OR specific_performance_available AND individual_thing_claim_available OR specific_performance_available AND article_395_interest_available OR specific_performance_available AND indemnity_prerequisites_satisfied OR substitute_performance_cost_recovery AND individual_thing_claim_available OR substitute_performance_cost_recovery AND article_395_interest_available OR substitute_performance_cost_recovery AND indemnity_prerequisites_satisfied OR individual_thing_claim_available AND article_395_interest_available OR individual_thing_claim_available AND indemnity_prerequisites_satisfied OR article_395_interest_available AND indemnity_prerequisites_satisfied",
            "counterparty_specific_claim_barred == reciprocal_obligations AND NOT own_counterperformance_tendered AND NOT specific_claim_override",
            "counterperformance_refusal_available == reciprocal_obligations AND counterperformance_due AND counterparty_failed_due_performance AND refusal_notice_delivered",
            "counterperformance_suspension_available == reciprocal_obligations AND counterperformance_due AND (counterparty_failed_due_performance OR clear_future_nonperformance) AND suspension_notice_delivered",
            "creditor_delay_damages_issue == creditor_in_delay AND creditor_delay_loss_proven",
            "creditor_extra_expense_liability_issue == extra_expenses_caused_by_creditor",
            "creditor_fault_reduction_issue == breach_established AND (creditor_contributed_to_loss OR NOT creditor_mitigation_taken)",
            "creditor_in_delay == creditor_refused_proper_performance OR creditor_omitted_required_action OR creditor_prerequisite_delay",
            "creditor_prerequisite_delay == creditor_prerequisite_action_required AND NOT creditor_prerequisite_action_completed",
            "current_price_damages == breach_established AND NOT replacement_transaction_made AND current_price_available",
            "damages_prerequisites_satisfied == breach_established AND loss_claimed AND (actual_loss_proven OR lost_profit_claimed AND lost_profit_measures_proven) AND causation_proven AND reasonable_amount_basis",
            "debtor_allocation_effective == multiple_homogeneous_debts AND payment_insufficient AND debtor_designated_debt AND debt_designation_valid",
            "debtor_answers_for_third_party == third_party_caused_breach AND debtor_responsible_for_third_party",
            "debtor_delay_excluded_by_creditor == debtor_delay AND creditor_in_delay",
            "debtor_in_delay == debtor_delay AND NOT creditor_in_delay",
            "default_payment_allocation_issue == multiple_homogeneous_debts AND payment_insufficient AND NOT debtor_allocation_effective",
            "demand_obligation_due == demand_obligation AND creditor_demand_delivered AND statutory_or_agreed_grace_elapsed",
            "early_performance_permitted == early_performance_tendered AND (NOT all_parties_acting_in_business OR early_performance_allowed)",
            "excess_monetary_damages_available == article_395_interest_available AND damages_above_interest_claimed AND damages_above_interest_proven",
            "facultative_substitute_effective == facultative_obligation AND facultative_substitute_tendered",
            "indemnity_bad_faith_bar == indemnity_agreement AND indemnity_bad_faith_event_caused",
            "indemnity_prerequisites_satisfied == indemnity_agreement AND indemnity_business_context AND indemnity_clear AND indemnity_trigger_unrelated_to_breach AND indemnity_loss_occurred AND indemnity_amount_or_method_agreed AND NOT indemnity_bad_faith_bar",
            "individual_thing_claim_available == individual_specific_thing_due AND NOT thing_transferred_to_protected_third_party",
            "intentional_liability_exclusion_invalid == intentional_breach AND advance_intentional_liability_exclusion",
            "judicial_loss_estimation_required == damages_prerequisites_satisfied AND exact_amount_not_established",
            "liability_limit_issue == breach_established AND liability_limit_clause_or_law",
            "lost_profit_supported == damages_prerequisites_satisfied AND lost_profit_claimed AND lost_profit_measures_proven",
            "partial_performance_acceptance_required == partial_performance_tendered AND (partial_performance_allowed OR monetary_obligation)",
            "partial_performance_refusal_available == partial_performance_tendered AND NOT (partial_performance_allowed OR monetary_obligation)",
            "proper_performance == obligation_exists AND performance_tendered AND subject_conforms AND quality_quantity_conform AND performance_at_due_time AND performance_at_proper_place AND performance_to_proper_recipient AND NOT partial_performance_tendered",
            "recipient_authority_gap == performance_tendered AND debtor_requested_authority_proof AND NOT authority_proof_provided",
            "replacement_transaction_damages == breach_established AND replacement_transaction_made AND replacement_transaction_reasonable",
            "requires_human_performance_remedies_assessment == recipient_authority_gap OR third_party_performance_acceptance_required OR default_payment_allocation_issue OR solidary_obligation OR counterperformance_suspension_available OR counterperformance_refusal_available OR counterparty_specific_claim_barred OR damages_prerequisites_satisfied OR judicial_loss_estimation_required OR creditor_fault_reduction_issue OR specific_performance_available OR article_395_interest_available OR article_395_penalty_conflict OR subsidiary_liability_prerequisites OR liability_limit_issue OR creditor_in_delay OR indemnity_prerequisites_satisfied OR indemnity_bad_faith_bar OR competing_remedy_paths",
            "solidary_external_discharge == solidary_obligation AND one_solidary_debtor_performed_full",
            "solidary_internal_recourse == solidary_external_discharge AND internal_recourse_shares_proven",
            "solidary_obligation == multiple_debtors AND (solidarity_by_law_or_contract OR joint_business_obligation)",
            "specific_performance_available == obligation_exists AND specific_performance_claimed AND performance_objectively_possible AND NOT creditor_lost_interest_due_delay",
            "subsidiary_liability_prerequisites == primary_debtor_claimed AND primary_refused_or_no_response AND subsidiary_debtor_claimed",
            "substitute_performance_cost_recovery == breach_established AND substitute_performance_by_creditor AND substitute_costs_reasonable_documented",
            "third_party_performance_acceptance_required == third_party_performance_tendered AND (debtor_assigned_third_party_performance OR debtor_monetary_delay OR third_party_property_right_at_risk) AND NOT personal_performance_required",
            "third_party_subrogation_issue == third_party_performance_acceptance_required",
        ],
    )

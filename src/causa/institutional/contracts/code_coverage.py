...
CODE_GAP_REASONS_RU: dict[str, tuple[str, str]] = {
    ...
    "165.1": (
        "пробел",
        "Юридически значимые сообщения. Это правило лежит под доброй половиной "
        "модели: в предикатах пакета больше двадцати фактов о доставленном "
        "уведомлении — об отказе от договора, о зачёте, о приостановлении "
        "встречного исполнения, о прощении долга. Когда сообщение считается "
        "доставленным, отвечает статья 165.1, и её в модели нет: каждый такой "
        "предикат принимает доставку на веру от того, кто заполняет факты.",
    ),
    "306": (
        "пробел",
    ...
    "860.1–860.15": (
        "пробел",
        "Специальные виды банковских счетов: номинальный счёт, счёт эскроу, "
        "публичный депозитный счёт, совместный счёт. Пятнадцать статей внутри "
        "главы 45, которую институт банковского счёта заявляет своей: он "
        "объявил 845–860 и остановился ровно перед ними. Самый крупный пробел "
        "обхода и самый показательный — он лежит не за границей модели, а "
        "внутри объявленного института.",
    ),
    "926.1–926.8": (
        "пробел",

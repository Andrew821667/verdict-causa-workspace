...
CODE_GAP_REASONS_RU: dict[str, tuple[str, str]] = {
    # Точный ключ важнее диапазона: без него статьи 25 и 65 попадали бы
    # (25 — под общий пробел главы 4/5, 65 — под запись «53.1–123.28» ниже) и
    # звучали бы как «корпоративное право не моделируется», хотя на деле обе
    # статьи — не о корпоративном праве, а об отсылке к отдельному закону о
    # банкротстве (127-ФЗ), которым занимается институт банкротства.
    "25": (
        "граница",
        "Несостоятельность (банкротство) гражданина: статья отсылает к "
        "отдельному Федеральному закону от 26.10.2002 № 127-ФЗ. Пакет "
        "моделирует эту область отдельным институтом банкротства, а не "
        "институтом лиц.",
    ),
    "65": (
        "граница",
        "Несостоятельность (банкротство) юридического лица: статья отсылает к "
        "тому же 127-ФЗ. Это не вопрос создания, реорганизации или ликвидации "
        "юридического лица (см. запись ниже для 53.1–123.28), а отдельная тема, "
        "которую пакет моделирует институтом банкротства.",
    ),
    "53.1–123.28": (
        "граница",

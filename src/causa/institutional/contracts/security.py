...
def build_security_constraint_set(
    ...
    return SecurityConstraintSet(
        ...
        expressions=[
            "accessory_security_displaced_by_main_invalidity == main_obligation_invalid AND (penalty_agreed OR pledge_created OR suretyship_created OR payment_transferred_at_conclusion AND payment_identified_as_deposit_in_writing AND NOT deposit_nature_doubtful OR security_payment_agreed)",
            "competing_security_paths == penalty_security_enforceable AND pledge_foreclosure_prerequisites OR penalty_security_enforceable AND retention_available OR penalty_security_enforceable AND surety_enforceable OR penalty_security_enforceable AND guarantee_demand_payable OR penalty_security_enforceable AND deposit_retained OR penalty_security_enforceable AND deposit_return_double OR penalty_security_enforceable AND security_payment_credit_available OR pledge_foreclosure_prerequisites AND retention_available OR pledge_foreclosure_prerequisites AND surety_enforceable OR pledge_foreclosure_prerequisites AND guarantee_demand_payable OR pledge_foreclosure_prerequisites AND deposit_retained OR pledge_foreclosure_prerequisites AND deposit_return_double OR pledge_foreclosure_prerequisites AND security_payment_credit_available OR retention_available AND surety_enforceable OR retention_available AND guarantee_demand_payable OR retention_available AND deposit_retained OR retention_available AND deposit_return_double OR retention_available AND security_payment_credit_available OR surety_enforceable AND guarantee_demand_payable OR surety_enforceable AND deposit_retained OR surety_enforceable AND deposit_return_double OR surety_enforceable AND security_payment_credit_available OR guarantee_demand_payable AND deposit_retained OR guarantee_demand_payable AND deposit_return_double OR guarantee_demand_payable AND security_payment_credit_available OR deposit_retained AND deposit_return_double OR deposit_retained AND security_payment_credit_available OR deposit_return_double AND security_payment_credit_available",
            "deposit_losses_issue == deposit_proven AND main_obligation_exists AND NOT main_obligation_invalid AND (deposit_giver_responsible OR deposit_recipient_responsible)",
            "deposit_proven == payment_transferred_at_conclusion AND payment_identified_as_deposit_in_writing AND NOT deposit_nature_doubtful",
            "deposit_retained == main_obligation_exists AND NOT main_obligation_invalid AND deposit_proven AND deposit_giver_responsible",
            "deposit_return_double == main_obligation_exists AND NOT main_obligation_invalid AND deposit_proven AND deposit_recipient_responsible",
            "deposit_return_single == deposit_proven AND main_obligation_exists AND NOT main_obligation_invalid AND (performance_impossible_without_fault OR agreement_terminated_before_performance)",
            "guarantee_demand_payable == guarantee_formally_effective AND creditor_demand_made AND guarantee_demand_timely AND guarantee_demand_complies AND NOT beneficiary_abuse_proven",
            "guarantee_formally_effective == independent_guarantee_issued AND guarantor_eligible AND guarantee_reproducible_form AND guarantee_essential_terms_identifiable AND NOT guarantee_expired",
            "guarantee_refusal_ground == independent_guarantee_issued AND creditor_demand_made AND (guarantee_expired OR NOT guarantee_demand_timely OR NOT guarantee_demand_complies OR beneficiary_abuse_proven)",
            "guarantee_regress_issue == guarantee_payment_made AND main_obligation_exists",
            "guarantee_survives_main_invalidity == guarantee_formally_effective AND main_obligation_invalid",
            "main_obligation_unaffected_by_security_defect == main_obligation_exists AND NOT main_obligation_invalid AND (penalty_agreed AND NOT penalty_writing_observed OR pledge_created AND NOT pledge_valid_between_parties OR suretyship_created AND NOT suretyship_writing_observed OR independent_guarantee_issued AND NOT guarantee_formally_effective)",
            "payment_treated_as_advance == payment_transferred_at_conclusion AND NOT deposit_proven",
            "penalty_security_enforceable == main_obligation_exists AND NOT main_obligation_invalid AND main_obligation_breached AND penalty_agreed AND penalty_writing_observed AND penalty_trigger_occurred",
            "pledge_extrajudicial_route == pledge_foreclosure_prerequisites AND extrajudicial_foreclosure_agreed AND extrajudicial_form_observed AND foreclosure_notice_delivered AND NOT judicial_foreclosure_mandatory",
            "pledge_foreclosure_bar == breach_insignificant AND secured_claim_manifestly_disproportionate",
            "pledge_foreclosure_prerequisites == pledge_valid_between_parties AND main_obligation_breached AND foreclosure_ground_exists AND secured_amount_proven AND NOT pledge_foreclosure_bar",
            "pledge_judicial_route == pledge_foreclosure_prerequisites AND (judicial_foreclosure_mandatory OR NOT pledge_extrajudicial_route)",
            "pledge_opposable_to_third_parties == pledge_valid_between_parties AND (NOT pledge_registration_required OR pledge_registration_completed OR third_party_knew_pledge)",
            "pledge_valid_between_parties == main_obligation_exists AND NOT main_obligation_invalid AND pledge_created AND pledge_form_observed AND pledged_asset_identified AND pledgor_owns_or_authorized",
            "requires_human_security_assessment == security_mechanism_detected OR penalty_agreed OR pledge_created OR suretyship_created OR payment_transferred_at_conclusion AND payment_identified_as_deposit_in_writing AND NOT deposit_nature_doubtful OR security_payment_agreed OR independent_guarantee_issued OR asset_lawfully_in_creditor_possession OR creditor_demand_made OR pledge_foreclosure_bar OR competing_security_paths",
            "retention_available == main_obligation_exists AND NOT main_obligation_invalid AND asset_lawfully_in_creditor_possession AND asset_return_due AND retention_secured_claim_due AND (claim_related_to_asset OR parties_acting_in_business)",
            "retention_enforcement_issue == retention_available AND creditor_demand_made",
            "security_enforcement_available == creditor_good_faith AND (penalty_security_enforceable OR pledge_foreclosure_prerequisites OR retention_available OR surety_enforceable OR guarantee_demand_payable OR deposit_retained OR deposit_return_double OR security_payment_credit_available)",
            "security_mechanism_detected == penalty_security_enforceable OR pledge_valid_between_parties OR retention_available OR surety_enforceable OR guarantee_formally_effective OR deposit_proven OR security_payment_active",
            "security_payment_active == main_obligation_exists AND NOT main_obligation_invalid AND security_payment_agreed AND security_payment_funded",
            "security_payment_credit_available == security_payment_active AND secured_circumstance_occurred AND NOT security_payment_credited",
            "security_payment_return_issue == main_obligation_exists AND NOT main_obligation_invalid AND security_payment_funded AND security_payment_return_condition_occurred AND NOT security_payment_credited",
            "surety_enforceable == main_obligation_exists AND NOT main_obligation_invalid AND main_obligation_breached AND suretyship_created AND suretyship_writing_observed AND surety_scope_proven AND NOT surety_terminated",
            "surety_regress_issue == surety_performed AND main_obligation_exists",
            "surety_scope_change_excluded == suretyship_created AND obligation_changed_increases_surety AND NOT surety_consented_to_change",
            "surety_solidary_liability == surety_enforceable AND NOT surety_subsidiary_agreed",
            "surety_subsidiary_liability == surety_enforceable AND surety_subsidiary_agreed",
            "surety_terminated == suretyship_created AND (surety_term_expired OR creditor_refused_proper_performance OR debt_transferred AND NOT surety_consented_to_new_debtor)",
        ],
    )

...
def build_obligation_dynamics_constraint_set(
    ...
    return ObligationDynamicsConstraintSet(
        ...
        expressions=[
            "accord_agreement_only == obligation_exists AND accord_agreed AND accord_form_observed AND NOT accord_performance_provided",
            "accord_discharge == obligation_exists AND accord_agreed AND accord_form_observed AND accord_performance_provided",
            "accrued_claims_preserved == accrued_claims_exist AND obligation_breached AND (obligation_discharged_full OR obligation_partially_remaining) AND NOT novation_effective AND NOT debt_forgiveness_effective",
            "assignment_contract_breach_issue == assignment_effective AND contract_restricts_assignment",
            "assignment_effective == obligation_exists AND assignment_agreement_concluded AND assignment_form_observed AND (assigned_claim_exists AND assigned_claim_identified OR future_claim_determinable) AND NOT claim_personal_to_creditor AND NOT assignment_prohibited_by_law AND (NOT debtor_consent_required OR debtor_consent_obtained) AND NOT claim_invalid",
            "assignment_enforceable_against_debtor == assignment_effective AND debtor_notified AND proof_of_transfer_provided",
            "cedent_document_transfer_issue == assignment_effective AND NOT cedent_transferred_documents",
            "cedent_invalid_claim_warranty_issue == assignment_agreement_concluded AND claim_invalid",
            "cedent_performance_guarantee_issue == assignment_effective AND cedent_guaranteed_debtor_performance AND debtor_failed_after_assignment",
            "competing_discharge_paths == proper_performance_discharge AND notary_deposit_discharge OR proper_performance_discharge AND accord_discharge OR proper_performance_discharge AND setoff_full_discharge OR proper_performance_discharge AND novation_effective OR proper_performance_discharge AND debt_forgiveness_effective OR proper_performance_discharge AND merger_discharge OR proper_performance_discharge AND impossibility_discharge OR proper_performance_discharge AND government_act_discharge OR proper_performance_discharge AND death_discharge OR proper_performance_discharge AND liquidation_discharge OR proper_performance_discharge AND other_discharge_effective OR notary_deposit_discharge AND accord_discharge OR notary_deposit_discharge AND setoff_full_discharge OR notary_deposit_discharge AND novation_effective OR notary_deposit_discharge AND debt_forgiveness_effective OR notary_deposit_discharge AND merger_discharge OR notary_deposit_discharge AND impossibility_discharge OR notary_deposit_discharge AND government_act_discharge OR notary_deposit_discharge AND death_discharge OR notary_deposit_discharge AND liquidation_discharge OR notary_deposit_discharge AND other_discharge_effective OR accord_discharge AND setoff_full_discharge OR accord_discharge AND novation_effective OR accord_discharge AND debt_forgiveness_effective OR accord_discharge AND merger_discharge OR accord_discharge AND impossibility_discharge OR accord_discharge AND government_act_discharge OR accord_discharge AND death_discharge OR accord_discharge AND liquidation_discharge OR accord_discharge AND other_discharge_effective OR setoff_full_discharge AND novation_effective OR setoff_full_discharge AND debt_forgiveness_effective OR setoff_full_discharge AND merger_discharge OR setoff_full_discharge AND impossibility_discharge OR setoff_full_discharge AND government_act_discharge OR setoff_full_discharge AND death_discharge OR setoff_full_discharge AND liquidation_discharge OR setoff_full_discharge AND other_discharge_effective OR novation_effective AND debt_forgiveness_effective OR novation_effective AND merger_discharge OR novation_effective AND impossibility_discharge OR novation_effective AND government_act_discharge OR novation_effective AND death_discharge OR novation_effective AND liquidation_discharge OR novation_effective AND other_discharge_effective OR debt_forgiveness_effective AND merger_discharge OR debt_forgiveness_effective AND impossibility_discharge OR debt_forgiveness_effective AND government_act_discharge OR debt_forgiveness_effective AND death_discharge OR debt_forgiveness_effective AND liquidation_discharge OR debt_forgiveness_effective AND other_discharge_effective OR merger_discharge AND impossibility_discharge OR merger_discharge AND government_act_discharge OR merger_discharge AND death_discharge OR merger_discharge AND liquidation_discharge OR merger_discharge AND other_discharge_effective OR impossibility_discharge AND government_act_discharge OR impossibility_discharge AND death_discharge OR impossibility_discharge AND liquidation_discharge OR impossibility_discharge AND other_discharge_effective OR government_act_discharge AND death_discharge OR government_act_discharge AND liquidation_discharge OR government_act_discharge AND other_discharge_effective OR death_discharge AND liquidation_discharge OR death_discharge AND other_discharge_effective OR liquidation_discharge AND other_discharge_effective",
            "contract_transfer_effective == obligation_exists AND contract_transfer_agreed AND all_parties_consented_contract_transfer AND assignment_form_observed AND debt_transfer_form_observed",
            "creditor_caused_impossibility_issue == impossibility_discharge AND creditor_caused_impossibility",
            "creditor_confirmation_issue == performance_rendered AND creditor_refused_confirmation AND NOT (creditor_issued_receipt OR creditor_returned_debt_instrument)",
            "cumulative_debtors == debt_transfer_effective AND NOT original_debtor_released AND (cumulative_debt_assumption_agreed OR business_debt_assumption)",
            "death_discharge == obligation_exists AND obligation_personal_to_deceased AND (personal_debtor_died OR personal_creditor_died)",
            "debt_forgiveness_effective == obligation_exists AND debt_forgiveness_declared AND debt_forgiveness_notice_delivered AND NOT debtor_objected_forgiveness AND NOT third_party_rights_prejudiced AND NOT forgiveness_gift_bar",
            "debt_transfer_effective == obligation_exists AND debt_transfer_agreement_concluded AND debt_transfer_form_observed AND new_debtor_identified AND creditor_consented_debt_transfer",
            "debtor_defenses_preserved == assignment_effective AND debtor_defense_existed_at_notice",
            "debtor_original_performance_discharges == assignment_effective AND debtor_performed_original_before_notice",
            "debtor_setoff_against_assignee_issue == assignment_effective AND debtor_counterclaim_existed_at_notice",
            "forgiveness_gift_bar == forgiveness_gift_intent AND commercial_parties",
            "government_act_discharge == obligation_exists AND government_act_prevents_performance AND NOT government_act_invalidated",
            "government_act_losses_issue == government_act_discharge",
            "government_act_restoration_issue == government_act_prevents_performance AND government_act_invalidated",
            "impossibility_discharge == obligation_exists AND objective_permanent_impossibility AND NOT impossibility_risk_on_debtor AND NOT debtor_in_delay_at_impossibility",
            "liquidation_discharge == obligation_exists AND legal_entity_liquidated AND NOT statutory_successor_exists",
            "merger_discharge == obligation_exists AND merger_creditor_and_debtor",
            "new_debtor_defenses_preserved == debt_transfer_effective AND new_debtor_defense_exists",
            "new_obligation_created_by_novation == novation_effective",
            "notary_deposit_discharge == obligation_exists AND notary_or_court_deposit_made AND deposit_ground_creditor_absent_or_evasive",
            "notary_deposit_notice_issue == notary_deposit_discharge AND NOT deposit_notice_sent",
            "novation_effective == obligation_exists AND novation_agreed AND novation_intent_clear AND new_subject_or_basis AND new_obligation_terms_agreed AND novation_form_observed",
            "obligation_discharged_full == (proper_performance_discharge OR notary_deposit_discharge OR accord_discharge OR setoff_full_discharge OR novation_effective OR debt_forgiveness_effective OR merger_discharge OR impossibility_discharge OR government_act_discharge OR death_discharge OR liquidation_discharge OR other_discharge_effective) AND NOT partial_termination_intended",
            "obligation_partially_remaining == obligation_exists AND (partial_performance_discharge OR setoff_partial_discharge OR partial_termination_intended) AND NOT obligation_discharged_full",
            "original_accessories_terminated_by_novation == novation_effective AND third_party_security_exists AND NOT third_party_security_consented_novation",
            "original_debtor_released_effective == debt_transfer_effective AND original_debtor_released",
            "other_discharge_effective == obligation_exists AND other_discharge_ground_proven",
            "partial_performance_discharge == obligation_exists AND performance_rendered AND performance_accepted_as_proper AND performance_partial",
            "parties_changed_not_discharged == (assignment_effective OR debt_transfer_effective OR contract_transfer_effective) AND NOT obligation_discharged_full",
            "proper_performance_discharge == obligation_exists AND performance_rendered AND performance_accepted_as_proper AND NOT performance_partial",
            "requires_human_dynamics_assessment == assignment_effective OR debt_transfer_effective OR contract_transfer_effective OR notary_deposit_discharge OR accord_agreement_only OR accord_discharge OR setoff_effective OR setoff_bar OR novation_effective OR debt_forgiveness_effective OR merger_discharge OR impossibility_discharge OR government_act_discharge OR death_discharge OR liquidation_discharge OR other_discharge_effective OR creditor_confirmation_issue OR cedent_invalid_claim_warranty_issue OR competing_discharge_paths",
            "setoff_bar == set_off_prohibited OR active_claim_limitation_expired",
            "setoff_effective == setoff_prerequisites AND set_off_declared AND set_off_notice_delivered",
            "setoff_full_discharge == setoff_effective AND claims_equal_amount",
            "setoff_partial_discharge == setoff_effective AND NOT claims_equal_amount",
            "setoff_prerequisites == obligation_exists AND counterclaims_mutual AND counterclaims_homogeneous AND active_claim_due AND passive_claim_due_or_early_allowed AND set_off_amount_proven AND NOT setoff_bar",
            "third_party_security_survives_novation == novation_effective AND third_party_security_exists AND third_party_security_consented_novation",
            "third_party_security_terminated_on_transfer == debt_transfer_effective AND third_party_security_exists AND NOT security_provider_consented_new_debtor",
        ],
    )

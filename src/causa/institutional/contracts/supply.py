...
def build_supply_constraint_set(mapping: SupplyEvidenceMappingResult) -> SupplyConstraintSet:
    return SupplyConstraintSet(
        ...
        expressions=[
            "abstract_current_price_damages_available == contract_terminated AND NOT replacement_transaction_made AND contract_price_proven AND current_price_available AND current_price_proven AND current_price_time_place_adjusted AND loss_claimed AND causation_proven",
            "acceptance_duties_satisfied == buyer_received_goods AND inspection_timely AND (NOT discrepancy_found OR prompt_written_notice)",
            "acceptance_inspection_gap == buyer_received_goods AND NOT inspection_timely",
            "acceptance_notice_gap == buyer_received_goods AND discrepancy_found AND NOT prompt_written_notice",
            "agreed_delivery_schedule_controls == periodic_delivery AND delivery_periods_agreed AND delivery_schedule_agreed",
            "buyer_disposal_available == responsible_custody_duties_satisfied AND NOT supplier_disposed_reasonable_time AND buyer_realized_or_returned_goods",
            "buyer_material_breach == repeated_payment_default OR repeated_selection_failure",
            "buyer_payment_claim_available == consignee_payment_duty AND consignee_failed_payment AND NOT buyer_paid",
            "completeness_remedies_available == incomplete_goods AND NOT incompleteness_promptly_cured_or_replaced",
            "completeness_remedies_displaced_by_prompt_cure == incomplete_goods AND incompleteness_promptly_cured_or_replaced",
            "concrete_price_damages_available == contract_terminated AND replacement_transaction_made AND replacement_transaction_reasonable AND replacement_transaction_timely AND contract_price_proven AND replacement_price_proven AND loss_claimed AND causation_proven",
            "container_return_breach == returnable_container AND container_return_due AND NOT container_returned",
            "cover_purchase_cost_recovery == (quantity_shortfall AND NOT actual_replenishment_completed OR quality_defect AND NOT defect_promptly_cured_or_replaced OR incomplete_goods AND NOT incompleteness_promptly_cured_or_replaced) AND buyer_acquired_substitute AND substitute_expenses_reasonable_documented",
            "cross_assortment_offset_barred == cross_assortment_offset AND NOT buyer_consented_offset",
            "default_contract_allocation_required == multiple_supply_contracts AND NOT performance_allocation_designated",
            "delivery_term_requires_general_default == supply_contract_qualified AND NOT transfer_term_defined AND NOT periodic_delivery",
            "early_delivery_counts_next_period == early_delivery AND buyer_accepted_early_delivery",
            "early_delivery_permitted == early_delivery AND buyer_consented_early_delivery",
            "late_makeup_refusal_effective == quantity_shortfall AND buyer_refused_late_makeup_by_notice",
            "makeup_assortment_default_to_prior_period == quantity_shortfall AND NOT makeup_assortment_agreed AND prior_period_assortment_proven",
            "makeup_delivery_required == quantity_shortfall AND contract_term_continues AND NOT buyer_refused_late_makeup_by_notice",
            "monthly_delivery_default == periodic_delivery AND NOT delivery_periods_agreed",
            "negotiation_loss_issue == negotiation_response_breach AND loss_claimed AND causation_proven",
            "negotiation_response_breach == disagreements_received AND NOT thirty_day_response_or_refusal",
            "other_damages_preserved == contract_terminated AND loss_claimed AND causation_proven AND other_loss_proven",
            "payment_withholding_available == (quality_defect AND NOT defect_promptly_cured_or_replaced OR incomplete_goods AND NOT incompleteness_promptly_cured_or_replaced) AND payment_withheld",
            "penalty_continues_until_replenishment == penalty_applies_to_short_or_late AND replenishment_duty_continues AND NOT actual_replenishment_completed",
            "quality_remedies_available == quality_defect AND NOT defect_promptly_cured_or_replaced",
            "quality_remedies_displaced_by_prompt_cure == quality_defect AND defect_promptly_cured_or_replaced",
            "requires_human_supply_assessment == delivery_term_requires_general_default OR negotiation_response_breach OR negotiation_loss_issue OR shipment_order_default OR shipment_order_loss_issue OR unauthorized_early_delivery OR makeup_delivery_required OR cross_assortment_offset_barred OR makeup_assortment_default_to_prior_period OR acceptance_inspection_gap OR acceptance_notice_gap OR transport_acceptance_issue OR responsible_custody_gap OR supplier_custody_cost_liability OR buyer_disposal_available OR supplier_payment_claim_after_unlawful_refusal OR selection_supplier_remedy_available OR buyer_payment_claim_available OR quality_remedies_available OR completeness_remedies_available OR cover_purchase_cost_recovery OR payment_withholding_available OR penalty_continues_until_replenishment OR default_contract_allocation_required OR supply_unilateral_refusal_effective OR concrete_price_damages_available OR abstract_current_price_damages_available OR other_damages_preserved",
            "responsible_custody_duties_satisfied == lawful_refusal AND goods_preserved AND supplier_notified",
            "responsible_custody_gap == lawful_refusal AND (NOT goods_preserved OR NOT supplier_notified)",
            "retail_purchaser_direct_demands == retail_purchaser_claim",
            "selection_supplier_remedy_available == buyer_failed_selection AND supplier_notified_readiness",
            "shipment_order_default == shipment_order_required AND NOT shipment_order_timely_complete",
            "shipment_order_loss_issue == shipment_order_default AND loss_claimed AND causation_proven",
            "supplier_custody_cost_liability == responsible_custody_duties_satisfied AND custody_expenses_documented",
            "supplier_material_breach == repeated_late_delivery OR irremediable_defect",
            "supplier_payment_claim_after_unlawful_refusal == unlawful_refusal AND supplier_demanded_payment_after_refusal AND NOT buyer_paid",
            "supplier_transport_choice == NOT transport_method_agreed AND supplier_selected_transport",
            "supply_breach_established == delivery_late OR unauthorized_early_delivery OR quantity_shortfall OR quality_defect OR incomplete_goods OR shipment_order_default OR container_return_breach OR consignee_failed_payment OR buyer_failed_selection OR responsible_custody_gap OR unlawful_refusal",
            "supply_contract_qualified == contract_concluded AND supplier_business AND supplier_produced_or_procured_goods AND goods_nonpersonal_use AND NOT retail_sale_context",
            "supply_unilateral_refusal_effective == contract_concluded AND (supplier_material_breach OR buyer_material_breach) AND unilateral_refusal_notice_delivered",
            "transport_acceptance_issue == documents_mismatch OR consignee_refused_goods",
            "unauthorized_early_delivery == early_delivery AND NOT buyer_consented_early_delivery",
        ],
    )

...
def build_sale_constraint_set(mapping: SaleEvidenceMappingResult) -> SaleConstraintSet:
    return SaleConstraintSet(
        ...
        expressions=[
            "assortment_deemed_accepted == assortment_nonconforming AND NOT buyer_notified_assortment_refusal_reasonable_time",
            "assortment_remedies_available == assortment_nonconforming",
            "buyer_acceptance_breach == buyer_failed_acceptance",
            "buyer_eviction_procedure_gap == third_party_eviction_claim_filed AND NOT buyer_joined_seller_to_eviction_case",
            "completeness_primary_remedies == incomplete_goods AND (buyer_chose_incomplete_price_reduction OR buyer_requested_completion)",
            "completeness_secondary_remedies == incomplete_goods AND buyer_requested_completion AND NOT seller_completed_reasonable_time AND (buyer_requested_complete_replacement OR buyer_chose_contract_refusal_for_incompleteness)",
            "credit_goods_security_active == credit_sale AND seller_security_interest_applies",
            "credit_payment_default == credit_sale AND credit_payment_due AND NOT credit_payment_made",
            "credit_seller_remedies == credit_sale AND credit_payment_due AND NOT credit_payment_made AND seller_demanded_credit_payment_or_return",
            "defect_causation_requirement_satisfied == quality_defect AND (seller_warranty_given AND warranty_period_active AND NOT seller_proved_posttransfer_defect_cause OR NOT (seller_warranty_given AND warranty_period_active) AND buyer_proved_pretransfer_defect_cause)",
            "defect_claim_timely == quality_defect AND defect_discovered_within_applicable_period",
            "documents_refusal_ground == (accessories_required AND NOT accessories_transferred OR documents_required AND NOT documents_transferred) AND buyer_set_reasonable_document_term AND seller_failed_document_term AND buyer_refused_goods_for_documents",
            "documents_remedy_available == (accessories_required AND NOT accessories_transferred OR documents_required AND NOT documents_transferred) AND buyer_set_reasonable_document_term",
            "eviction_exclusion_ineffective == third_party_rights_exist AND seller_eviction_exclusion_agreement",
            "eviction_loss_remedy_available == goods_withdrawn_by_third_party AND withdrawal_ground_predates_transfer AND NOT buyer_knew_withdrawal_ground AND NOT (third_party_eviction_claim_filed AND NOT buyer_joined_seller_to_eviction_case AND seller_could_prevent_withdrawal) AND loss_claimed AND causation_proven",
            "excess_quantity_deemed_accepted == excess_quantity AND buyer_notified_excess AND NOT seller_disposed_excess_reasonable_time AND buyer_accepted_excess",
            "excess_quantity_payment_due == excess_quantity AND buyer_accepted_excess",
            "goods_transfer_refusal_remedy_available == seller_refused_goods_transfer",
            "installment_refusal_available == installment_sale AND installment_payment_due AND NOT installment_payment_made AND seller_chose_installment_refusal AND NOT paid_amount_exceeds_half_price",
            "installment_refusal_barred_by_half_payment == installment_sale AND installment_payment_due AND NOT installment_payment_made AND seller_chose_installment_refusal AND paid_amount_exceeds_half_price",
            "installment_terms_gap == installment_sale AND NOT installment_essential_terms_complete",
            "insurance_breach == insurance_duty_allocated AND insurance_duty_due AND NOT insurance_obtained",
            "insurance_refusal_available == insurance_duty_allocated AND insurance_duty_due AND NOT insurance_obtained AND counterparty_chose_insurance_refusal",
            "insurance_self_help_available == insurance_duty_allocated AND insurance_duty_due AND NOT insurance_obtained AND counterparty_insured_goods",
            "late_fixed_term_transfer_requires_consent == delivery_late AND fixed_term_contract AND buyer_interest_lost_after_term AND NOT buyer_consented_late_transfer",
            "material_defect_remedies_available == quality_defect AND defect_material AND (seller_warranty_given AND warranty_period_active AND NOT seller_proved_posttransfer_defect_cause OR NOT (seller_warranty_given AND warranty_period_active) AND buyer_proved_pretransfer_defect_cause) AND defect_discovered_within_applicable_period AND (buyer_chose_replacement OR buyer_chose_contract_refusal_for_defect)",
            "mixed_assortment_total_refusal_available == assortment_nonconforming AND mixed_assortment_delivery AND buyer_rejected_all_mixed_goods AND buyer_notified_assortment_refusal_reasonable_time",
            "net_weight_price_controls == price_based_on_weight AND net_weight_proven",
            "notice_defense_available == discrepancy_found AND NOT prompt_notice_given AND seller_proved_notice_prejudice AND NOT seller_knew_or_should_have_known_discrepancy",
            "notice_defense_displaced_by_seller_knowledge == discrepancy_found AND NOT prompt_notice_given AND seller_knew_or_should_have_known_discrepancy",
            "packaging_remedies_available == goods_transfer_completed AND packaging_required AND (NOT packaging_transferred OR NOT packaging_conforming) AND (buyer_requested_packaging OR buyer_requested_packaging_replacement OR buyer_elected_other_packaging_remedy)",
            ...
            "quality_remedies_available == quality_defect AND (seller_warranty_given AND warranty_period_active AND NOT seller_proved_posttransfer_defect_cause OR NOT (seller_warranty_given AND warranty_period_active) AND buyer_proved_pretransfer_defect_cause) AND defect_discovered_within_applicable_period AND (buyer_chose_price_reduction OR buyer_chose_free_repair OR buyer_chose_repair_costs)",
            "quality_standard_requires_general_default == NOT quality_terms_agreed AND (ordinary_purpose_known OR special_purpose_disclosed OR sale_by_sample_or_description OR mandatory_quality_requirements_apply)",
            "quantity_shortfall_remedies == quantity_shortfall",
            "risk_passed_by_contract == risk_allocation_agreed AND risk_passed_by_agreement",
            "risk_passed_by_default == goods_transfer_completed AND (delivery_obligation AND goods_delivered_to_buyer OR NOT delivery_obligation AND NOT shipment_contract AND goods_made_available OR shipment_contract AND goods_handed_to_carrier) AND NOT risk_allocation_agreed",
            "sale_contract_qualified == contract_concluded AND seller_transfer_ownership_duty AND buyer_acceptance_duty AND buyer_payment_duty AND goods_existing_or_future AND goods_name_agreed AND quantity_determinable",
            "sale_contract_refusal_effective == contract_concluded AND (seller_refused_goods_transfer AND buyer_chose_contract_refusal_for_nontransfer OR (accessories_required AND NOT accessories_transferred OR documents_required AND NOT documents_transferred) AND buyer_set_reasonable_document_term AND seller_failed_document_term AND buyer_refused_goods_for_documents OR quality_defect AND defect_material AND (seller_warranty_given AND warranty_period_active AND NOT seller_proved_posttransfer_defect_cause OR NOT (seller_warranty_given AND warranty_period_active) AND buyer_proved_pretransfer_defect_cause) AND defect_discovered_within_applicable_period AND buyer_chose_contract_refusal_for_defect OR incomplete_goods AND buyer_requested_completion AND NOT seller_completed_reasonable_time AND buyer_chose_contract_refusal_for_incompleteness OR buyer_failed_acceptance AND seller_chose_contract_refusal_for_nonacceptance OR installment_sale AND installment_payment_due AND NOT installment_payment_made AND NOT paid_amount_exceeds_half_price AND seller_chose_installment_refusal OR insurance_duty_allocated AND insurance_duty_due AND NOT insurance_obtained AND counterparty_chose_insurance_refusal) AND unilateral_refusal_notice_delivered",
            "seller_assortment_choice_default == assortment_required AND NOT assortment_agreed AND assortment_determinable_from_needs AND seller_selected_assortment",
            "seller_eviction_defense_barred == third_party_eviction_claim_filed AND buyer_joined_seller_to_eviction_case AND NOT seller_participated_in_eviction_case",
            "seller_nonacceptance_remedies == buyer_failed_acceptance AND (seller_demanded_acceptance OR seller_chose_contract_refusal_for_nonacceptance)",
            "seller_payment_remedies == payment_due AND NOT buyer_paid AND seller_demanded_payment",
            "set_delivery_breach == set_of_goods_agreed AND transfer_term_due AND NOT set_transfer_complete",
            "shelf_life_transfer_breach == goods_transfer_completed AND shelf_life_set AND NOT goods_transferred_with_usable_shelf_life",
            "specific_performance_available == seller_refused_goods_transfer AND individually_defined_goods AND specific_performance_requested",
            "third_party_rights_breach == third_party_rights_exist AND NOT buyer_consented_third_party_rights",
            "title_disposal_bar == title_retention_agreed AND NOT title_condition_met AND buyer_disposed_before_title AND NOT title_early_disposal_permitted",
            "title_return_remedy == title_retention_agreed AND NOT title_condition_met AND seller_required_goods_return AND NOT title_return_contract_bar",
            "transfer_duty_performed == goods_transfer_completed AND (delivery_obligation AND goods_delivered_to_buyer OR NOT delivery_obligation AND NOT shipment_contract AND goods_made_available OR shipment_contract AND goods_handed_to_carrier)",
            "transfer_package_complete == goods_transfer_completed AND NOT (accessories_required AND NOT accessories_transferred OR documents_required AND NOT documents_transferred)",
            "transfer_package_gap == goods_transfer_completed AND (accessories_required AND NOT accessories_transferred OR documents_required AND NOT documents_transferred)",
            "transit_risk_clause_challenge == goods_sold_in_transit AND seller_knew_transit_loss AND NOT seller_disclosed_transit_loss",
            "transit_risk_passed_at_conclusion == goods_sold_in_transit AND NOT risk_allocation_agreed",
            "warranty_seller_burden_applies == quality_defect AND seller_warranty_given AND warranty_period_active",
        ],
    )
